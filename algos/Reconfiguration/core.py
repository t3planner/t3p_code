...
class Actor(nn.Module):
    ...
    def forward(self, obs, act=None):
        ...
        logp_a = None
        if act is not None:
            act = act.reshape((int(len(act) / 2), 2))
...
class MLPCategoricalActor(Actor):
    ...
    def __init__(self, obs_dim, act_dim, stru, hidden_sizes, activation, gcn_outdim, nhid=16):
        ...
        self.logits_net = mlp([obs_dim] + list(hidden_sizes) + [act_dim * 2], activation)
    ...
    def _distribution(self, obs):
        if self.stru == "dual-homing":
            ...
            X, Z, adj_e, adj_v, T, state_cost, state_flag, state_mesh = obs
            embedding = self.actor_gcn(X, Z, adj_e, adj_v, T)
            obs = torch.cat((F.log_softmax(state_cost.reshape((len(embedding), 1)), dim=0),
                             state_flag.reshape((len(embedding), 1)), state_mesh.reshape((len(embedding), 1)),
                             embedding), axis=1).flatten().float()
        elif self.stru == "hybrid ring-mesh":
            X, Z, adj_e, adj_v, T, state_cost, state_flag, state_dual, state_mesh = obs
            embedding = self.actor_gcn(X, Z, adj_e, adj_v, T)
            obs = torch.cat((F.log_softmax(state_cost.reshape((len(embedding), 1)), dim=0),
                             state_flag.reshape((len(embedding), 1)), state_dual.reshape((len(embedding), 1)),
                             state_mesh.reshape((len(embedding), 1)), embedding), axis=1).flatten().float()

        logits = self.logits_net(obs)

        return Categorical(logits=logits.reshape((2, self.act_dim)))
