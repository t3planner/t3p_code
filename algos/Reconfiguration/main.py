...
from algos.Reconfiguration.plan_env import GraphEnv
import networkx as nx
import pandas as pd
from algos.Reconfiguration.core import MLPActorCritic
from algos.traffic import update_traffic

def RL_congestion(dataset, cap, adjust_num, adjust_demand, stru, gcn_outdim, steps_per_epoch, epochs, max_ep_len, cuda):
    # construct dataset
    ...
    for i in range(len(fiber)):
        ...
        cost = fiber.iloc[i, 2]
        G_fiber.add_edge(node_i, node_j, cost=cost, cap=cap)
    ...
    for i in range(len(dis_matrix)):
        ...
        G_IP.add_edge(node_i, node_j, cost=0, load=0, cap=cap, fiber=[])
    ...
    for i in range(len(traffic_matrix)):
        src = int(traffic_matrix.iloc[i, 0])
        dst = int(traffic_matrix.iloc[i, 1])
    ...
    G_init = nx.create_empty_copy(G_IP)
    file_name = './datasets/{}/init.csv'.format(dataset)
    fiber_matrix = pd.read_csv(file_name, header=None)
    for i in range(node_num):
        for j in range(node_num):
            if fiber_matrix.iloc[i, j] == 1:
                G_init.add_edge(i, j, cost=G_IP[i][j]['cost'], load=0, cap=cap, fiber=[])

    env = GraphEnv(G_fiber, G_IP, traffic, SRLG, adjust_num, adjust_demand, stru, G_init, cuda)

    ac_kwargs = dict(gcn_outdim=gcn_outdim, adjust_num=adjust_num, stru=stru, cuda=cuda)
    logger_kwargs = dict(
        output_dir="result//Traffic based reconfiguration scenario//{}//{}".format(dataset, stru), output_fname='progress.txt'
    )
    ac = MLPActorCritic
    ppo("congestion", env, actor_critic=ac, ac_kwargs=ac_kwargs, logger_kwargs=logger_kwargs, steps_per_epoch=steps_per_epoch,epochs=epochs, max_ep_len=max_ep_len)