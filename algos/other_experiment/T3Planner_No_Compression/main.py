...
import numpy as np
from algos.other_experiment.T3Planner_No_Compression.plan_env import GraphEnv
import networkx as nx
import pandas as pd
from algos.other_experiment.T3Planner_No_Compression.core import MLPActorCritic
from algos.traffic import update_traffic

def RL_T3Planner_No_Compression(dataset, adjust_num, stru, gcn_outdim, steps_per_epoch, epochs, max_ep_len, cuda):
    ...
    for i in range(len(fiber)):
        ...
        cost = fiber.iloc[i, 2]
        if node_i not in G_fiber.nodes:
            G_fiber.add_node(node_i, type='other')
        if node_j not in G_fiber.nodes:
            G_fiber.add_node(node_j, type='other')
    ...
    G_ILP = None
    env = GraphEnv(G_fiber, G_IP, traffic, SRLG, adjust_num, stru, G_ILP, cuda)

    ac_kwargs = dict(gcn_outdim=gcn_outdim, adjust_num=adjust_num, stru=stru, cuda=cuda)
    logger_kwargs = dict(
        output_dir="result//T3Planner (No Compression)//{}//{}".format(dataset, stru), output_fname='progress.txt'
    )
    ac = MLPActorCritic
