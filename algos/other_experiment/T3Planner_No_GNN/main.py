...
import numpy as np
from algos.other_experiment.T3Planner_No_GNN.plan_env import GraphEnv
import networkx as nx
import pandas as pd
from algos.other_experiment.T3Planner_No_GNN.core import MLPActorCritic
from algos.traffic import update_traffic

def RL_T3Planner_No_GNN(dataset, adjust_num, stru, gcn_outdim, steps_per_epoch, epochs, max_ep_len, cuda):
    ...
    logger_kwargs = dict(
        output_dir="result//T3Planner(No GNN)//{}//{}".format(dataset, stru), output_fname='progress.txt'
    )
    ac = MLPActorCritic
