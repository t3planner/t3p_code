...
class MLPCategoricalActor(Actor):
    ...
    def _distribution(self, obs):
        state_cost, state_flag = obs
        obs = torch.cat((F.log_softmax(state_cost.reshape((len(state_cost), 1)), dim=0),
                             state_flag.reshape((len(state_cost), 1))), axis=1).flatten().float()
...
class MLPCritic(nn.Module):
    ...
    def forward(self, obs):
        state_cost, state_flag = obs
        obs = torch.cat(
            (F.log_softmax(state_cost.reshape((len(state_cost), 1)), dim=0), state_flag.reshape((len(state_cost), 1))),
            axis=1).flatten()
