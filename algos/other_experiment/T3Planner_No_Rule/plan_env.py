...
class GraphEnv(gym.Env):
    def __init__(
        ...
        _, _, _, self.original_heur_traffic_path, _ = Heur_cost(self.G_fiber, self.original_graph, traffic, self.SRLG)
        self.original_topo = copy.deepcopy(self.original_graph)
        # compute fiber cost
        self.original_topo_fiber = nx.Graph()
        for (node_i, node_j) in self.original_topo.edges:
            fiber_path = self.original_topo[node_i][node_j]['fiber']
            for k in range(1, len(fiber_path)):
                self.original_topo_fiber.add_edge(fiber_path[k - 1], fiber_path[k])
        obj = 0
        for (node_i, node_j) in self.original_topo_fiber.edges:
            obj += self.G_fiber[node_i][node_j]['cost']
        ...
        self.obj = obj
        self.done_topo = copy.deepcopy(self.original_topo)  # save the resulting graph for each trajectory
        self.done_topo_fiber = copy.deepcopy(self.original_topo_fiber)  # save the resulting graph for each trajectory
    ...
    def step(self, action, obs):
        ...
        if done == False:
            # determine whether the result meets the constraints
            flag, _, _, traffic_path, _ = Heur(self.stru, "cost", self.G_fiber, self.graph, self.traffic, self.SRLG)
            topo = copy.deepcopy(self.graph)
            # compute fiber cost
            topo_fiber = nx.Graph()
    ...
    def reset(self):
        ...
        self.done_topo = copy.deepcopy(self.original_topo)
        self.done_topo_fiber = copy.deepcopy(self.original_topo_fiber)
        if self.stru == "dual-homing" or self.stru == "hybrid ring-mesh":
            self.edge_to_ring = copy.deepcopy(self.original_edge_to_ring)
            self.edge_feature = copy.deepcopy(self.original_edge_feature)
        self.node_edge = copy.deepcopy(self.original_node_edge)
