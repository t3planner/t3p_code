...
from MLBO.solver import greedy_algorithm
...
def save_results(dataset, traffic_paths:list=None, fiber_net:nx.Graph=None, load_graph:nx.Graph=None):
    if traffic_paths is not None: 
        with open('./MLBO/results/{}_flow.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if fiber_net is not None:
        ...
        with open('./MLBO/results/{}_fiber_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if load_graph is not None:
        ...
        with open('./MLBO/results/{}_load_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
            for row in load_matrix:
                writer.writerow(row)


def MLBO(dataset, original_graph:nx.Graph, original_traffic:dict, original_SRLG:list, ring_len=10): 
    G = copy.deepcopy(original_graph)
    graph = copy.deepcopy(original_graph)
    traffic = copy.deepcopy(original_traffic)

    with open('./MLBO/results/results.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['dataset', 'total_time', 'cost', 'none_backup', 'max_load'])

    start_time = time.time()

    fiber_net, load_graph, traffic_paths = greedy_algorithm(G, graph, traffic, ring_len=ring_len)
