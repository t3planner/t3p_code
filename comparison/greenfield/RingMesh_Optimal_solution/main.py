...
from RingMesh_Optimal_solution.solver import linear_programing
...
def save_results(dataset, traffic_paths:list=None, fiber_net:nx.Graph=None, load_graph:nx.Graph=None):
    if traffic_paths is not None: 
        with open('./RingMesh_Optimal_solution/results/{}_flow.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if fiber_net is not None:
        ...
        with open('./RingMesh_Optimal_solution/results/{}_fiber_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if load_graph is not None:
        ...
        with open('./RingMesh_Optimal_solution/results/{}_load_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
            for row in load_matrix:
                writer.writerow(row)


def RingMesh_Optimal_solution(
    ...
    with open('./RingMesh_Optimal_solution/results/results.csv', 'a', newline='') as f:
        writer = csv.writer(f)
