...
from Optimal_solution.solver import linear_programing
...
def save_results(dataset, traffic_paths:list=None, fiber_net:nx.Graph=None, load_graph:nx.Graph=None):
    if traffic_paths is not None: 
        with open('./Optimal_solution/results/{}_flow.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if fiber_net is not None:
        ...
        with open('./Optimal_solution/results/{}_fiber_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if load_graph is not None:
        ...
        with open('./Optimal_solution/results/{}_load_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
            for row in load_matrix:
                writer.writerow(row)


def Optimal_solution(dataset, original_graph:nx.Graph, original_traffic:dict, original_SRLG:list, time_limit=None):
    ...
    with open('./Optimal_solution/results/results.csv', 'a', newline='') as f:
        writer = csv.writer(f)
    ...
    fiber_net, load_graph, traffic_paths = linear_programing(G, graph, traffic, SRLG_pair, time_limit=time_limit)
