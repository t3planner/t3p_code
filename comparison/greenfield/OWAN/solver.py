...
def annealing(SRLG_graph:nx.Graph, T, iters, alpha, traffic:dict, ring_len=10):
    E_0 = -T
    ...
    while True:
        ...
        for i in range(iters): 
            ...
            # state transition 
            if E_t < E_0: 
                if dual_num==agg_num and allocate_num==traffic_num:
                    SRLG_graph = graph
                    E_0 = E_t
                    traffic_paths = paths
...
def simulated_annealing(
    ...
    fiber_net, SRLG_traffic_paths = annealing(SRLG_graph, T, iters, alpha, original_traffic, ring_len)
