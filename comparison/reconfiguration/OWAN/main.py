...
from OWAN.solver import simulated_annealing
...
def save_results(dataset, traffic_paths:list=None, fiber_net:nx.Graph=None, load_graph:nx.Graph=None):
    if traffic_paths is not None: 
        with open('./OWAN/results/{}_flow.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if fiber_net is not None:
        ...
        with open('./OWAN/results/{}_fiber_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if load_graph is not None:
        ...
        with open('./OWAN/results/{}_load_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
...
def OWAN(
    dataset, original_graph:nx.Graph, init_graph:nx.Graph, original_traffic:dict, original_SRLG:list, 
    ...
    with open('./OWAN/results/results.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['dataset', 'total_time', 'cost', 'none_backup', 'max_load'])

    start_time = time.time()

    fiber_net, load_graph, traffic_paths = simulated_annealing(G, graph, traffic, alpha=alpha, division=division, ring_len=ring_len)
