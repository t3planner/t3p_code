...
def gurobi_solver(SRLG_graph:nx.Graph, traffic:dict, SRLG_pair:dict, max_nodes=10, max_rings=30, total_load=0, time_limit=None, verbose=True): 
    core = []
    dis = []
    agg = []
    ...
    weights = model.addVars(traffic_num, 2, lb=0, vtype=gurobipy.GRB.BINARY, name='weights')
    ...
    model.addConstr(sum(loads[node_i, node_j] for (node_i, node_j) in SRLG_graph.edges) == total_load)
    ...
    for (node_i, node_j) in edge_list: 
        ...
        # model.addConstr((edges[node_i, node_j]==0) >> (loads[node_i, node_j]==0))
        ...
        model.addConstr(loads[node_i, node_j] <= SRLG_graph[node_i][node_j]['cap'])
    ...
    if model.status == gurobipy.GRB.Status.OPTIMAL or model.status == gurobipy.GRB.Status.TIME_LIMIT:
        ...
        edge_loads = {}
        loads = model.getAttr('x', loads)
        for (node_i, node_j) in edge_list: 
            edge_loads[(node_i, node_j)] = loads[node_i, node_j]

        return fiber_net, traffic_paths, path_weights
...
def linear_programing(
    ...
    max_nodes=10, max_rings=30, total_load=0, time_limit=None
):
    fiber_net = nx.Graph()
    ...
    SRLG_fiber_net, SRLG_traffic_paths, path_weights = gurobi_solver(SRLG_graph, original_traffic, SRLG_pair, max_nodes, max_rings, total_load, time_limit)
    for (node_i, node_j) in SRLG_fiber_net.edges:
        if node_i in SRLG_pair.keys(): 
            if node_j != SRLG_pair[node_i]:
                if node_j in SRLG_pair.keys(): 
                    fiber_net.add_edge(SRLG_pair[node_i], SRLG_pair[node_j])
                else: 
    ...
    for (src, dst), paths in SRLG_traffic_paths.items(): 
        ...
        for j in range(len(paths)): 
            ...
            load_graph = update_graph(load_graph, paths[j], weight*demand)
