...
from SMORE.solver import traffic_engineering
...
def save_results(dataset, traffic_paths:list=None, fiber_net:nx.Graph=None, load_graph:nx.Graph=None):
    if traffic_paths is not None: 
        with open('./SMORE/results/{}_flow.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if fiber_net is not None:
        ...
        with open('./SMORE/results/{}_fiber_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
    ...
    if load_graph is not None:
        ...
        with open('./SMORE/results/{}_load_matrix.csv'.format(dataset), 'w', newline='') as f:
            writer = csv.writer(f)
...
def SMORE(
    dataset, k, original_graph:nx.Graph, original_traffic:dict, original_SRLG:list, time_limit=None
): 
    ...
    with open('./SMORE/results/results.csv', 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['dataset', 'k', 'total_time', 'cost', 'none_backup', 'max_load'])

    start_time = time.time()

    fiber_net, load_graph, traffic_paths = traffic_engineering(k, G, graph, traffic, SRLG_pair, time_limit)
